...
const executeTool = async (name, args, userId) => {
    ...
    switch (name) {
        case 'create_task': {
            ...
            // Find workspace
            let workspace = await Workspace_js_1.default.findOne({ 'members.user': userId });
            if (!workspace) {
                workspace = await Workspace_js_1.default.findOne();
            }
            if (!workspace)
            ...
            let project;
            if (project_id) {
                project = await Project_js_1.default.findById(project_id);
            }
            else {
                project = await Project_js_1.default.findOne({ workspaceId: workspace._id });
            }
            if (!project) {
                // Fallback to first available project
                project = await Project_js_1.default.findOne();
            }
            if (!project)
            ...
            if (assignee) {
                assigneeUser = await User_js_1.default.findOne({ name: new RegExp(assignee, 'i') });
                if (!assigneeUser) {
                    assigneeUser = await User_js_1.default.findById(assignee).catch(() => null);
                }
            }
            const task = await Task_js_1.default.create({
                title,
        ...
        case 'update_task': {
            ...
            const task = await Task_js_1.default.findById(task_id);
            if (!task)
            ...
            if (fields.assignee) {
                const assigneeUser = await User_js_1.default.findOne({ name: new RegExp(fields.assignee, 'i') });
                if (assigneeUser) {
                    task.assignee = assigneeUser._id;
                }
            }
            await task.save();
            const updated = await Task_js_1.default.findById(task_id)
            ...
            return {
                ...
                data: {
                    id: task._id.toString(),
                    title: task.title,
                    project: updated?.projectId?.name || 'General Project',
        ...
        case 'assign_task': {
            ...
            if (!task)
                throw new Error('Task not found');
            let targetUser = await User_js_1.default.findOne({ name: new RegExp(user_id, 'i') });
            if (!targetUser) {
                targetUser = await User_js_1.default.findById(user_id).catch(() => null);
            }
            if (!targetUser)
                throw new Error('Assignee user not found');
            task.assignee = targetUser._id;
            await task.save();
            const updated = await Task_js_1.default.findById(task_id).populate('projectId', 'name');
            // Broadcast Socket Update
            try {
                const io = (0, socket_service_js_1.getIO)();
                io.to(`workspace_${task.workspaceId.toString()}`).emit('task_updated', { taskId: task._id.toString() });
        ...
        case 'query_tasks': {
            ...
            if (filters.assignee) {
                const targetUser = await User_js_1.default.findOne({ name: new RegExp(filters.assignee, 'i') });
                if (targetUser) {
            ...
            if (filters.projectId) {
                ...
                const prj = await Project_js_1.default.findOne({ name: new RegExp(filters.project, 'i') });
                if (prj)
            ...
            const tasks = await Task_js_1.default.find(query)
                .populate('projectId', 'name')
            ...
            return {
                ...
                data: tasks.map(t => ({
                    ...
                    project: t.projectId?.name || 'General Project',
        ...
        case 'summarize_project': {
            ...
            let project = await Project_js_1.default.findById(project_id);
            if (!project) {
                project = await Project_js_1.default.findOne({ name: new RegExp(project_id, 'i') });
            }
            if (!project)
                throw new Error('Project not found');
            const tasks = await Task_js_1.default.find({ projectId: project._id });
        ...
        case 'flag_risks': {
            ...
            if (!project)
                throw new Error('Project not found');
            const tasks = await Task_js_1.default.find({ projectId: project._id }).populate('assignee', 'name');
            const today = new Date();
            ...
            return {
                ...
                data: {
                    projectName: project.name,
                    overdue: overdueTasks.map(t => ({ id: t._id.toString(), title: t.title, assignee: t.assignee?.name || 'Unassigned' })),
                    blocked: blockedTasks.map(t => ({ id: t._id.toString(), title: t.title, assignee: t.assignee?.name || 'Unassigned' })),
...
// ─── AI Chat Controllers ─────────────────────────────────────────────────────
const runSandboxAI = async (chatMessages, userId, errorMessage) => {
    ...
    if (msgText.includes('project') || msgText.includes('show projects')) {
        const projects = await Project_js_1.default.find({ workspaceId: { $in: workspaceIds } });
    ...
    if (msgText.includes('create task')) {
        ...
        // Get first project
        const project = await Project_js_1.default.findOne({ workspaceId: { $in: workspaceIds } });
        if (!project) {
...
const chatWithAI = async (req, res) => {
    ...
    try {
        ...
        const currentUser = userId ? await User_js_1.default.findById(userId) : null;
        // Fetch active workspace details
        let activeWorkspaceName = 'NexusAI Core';
        const workspaceId = context?.workspaceId;
        if (workspaceId) {
            const workspace = await Workspace_js_1.default.findById(workspaceId);
            if (workspace) {
                activeWorkspaceName = workspace.name;
            }
        }
        else if (userId) {
            const workspace = await Workspace_js_1.default.findOne({ 'members.user': userId });
            if (workspace) {
                activeWorkspaceName = workspace.name;
            }
        }
        ...
        if (context) {
            ...
            if (context.taskId) {
                const activeTask = await Task_js_1.default.findById(context.taskId).populate('projectId', 'name');
                if (activeTask) {
                    contextPrompt += `\n- Viewing Task: "${activeTask.title}" (ID: ${activeTask._id}, Status: ${activeTask.status}, Project: "${activeTask.projectId?.name}")`;
                }
            }
            if (context.projectId) {
                const activeProject = await Project_js_1.default.findById(context.projectId);
                if (activeProject) {
...
- Guardrails: Confirm before any destructive actions (like bulk deletions or reassigning multiple items at once). Only interact within the active workspace context.${contextPrompt}`;
        ...
        if (!success && process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_API_KEY.includes('placeholder')) {
            try {
                ...
                const toolUseBlock = contentBlocks.find((c) => c.type === 'tool_use');
                ...
                if (toolUseBlock) {
                    ...
                    try {
                        ...
                        const anthropicSecondRes = await fetch('https://api.anthropic.com/v1/messages', {
                            method: 'POST',
                            headers: {
                                'content-type': 'application/json',
                                'x-api-key': process.env.ANTHROPIC_API_KEY,
                        ...
                        if (anthropicSecondRes.ok) {
                            const secondData = await anthropicSecondRes.json();
                            const secondText = (secondData.content || []).find((c) => c.type === 'text');
                            if (secondText)
                                reply = secondText.text;
                        }
                    }
                    catch (err) {
